...
def test_sync_volume_level_uses_parent_volume_for_non_player_control(
    airplay_player: AirPlayPlayer,
) -> None:
    """Keep pulling the parent volume when its control is not a player (fake/external)."""
    parent = MagicMock()
    parent.state.volume_level = 42
    parent.volume_control = "fake"
    airplay_player.mass.players.get_player.side_effect = {  # type: ignore[attr-defined]
        "parent": parent,
    }.get
    airplay_player.set_protocol_parent_id("parent")
